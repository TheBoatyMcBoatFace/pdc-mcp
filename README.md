...
- **Richer, sentence-level descriptions** live only in CMS's per-provider data-dictionary **PDFs**
  (surfaced via `get_dataset`'s `dataDictionary` link). Generic auto-parsing was proven non-viable
  — the PDFs have incompatible layouts (the hospital dictionary is a 105-page narrative spec) and
  their labels diverge from the CSV headers. The clean structured source is generated by internal
  data teams and hard to obtain, so the realistic path is per-provider extraction from the files in
  `data_dictionaries/` (the Planfinder **XLSX** is already structured; clean-table PDFs need
  tailored parsing) → committed `src/dictionaries/<provider>.json` → attached by exact column name.
...
- ⏳ Rich column descriptions from `data_dictionaries/` (per-provider)
